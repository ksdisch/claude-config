...
### [Fix] retire: a retired loose copy must not make its plugin-served name look dangling
- **Why:** `dangling_names()` treats every id in the ledger's retired table as a missing route target, prefix stripped. After the 2026-09-21 sweep ledgered 25 loose mattpocock copies by bare name (`skill:code-review`, `skill:to-tickets`, …), every skill or CLAUDE.md section that names one of those — which the `mattpocock-skills` plugin still serves — carries a false `dangling` flag; 8 spurious `ask` rows appeared in the after-run.
- **Acceptance:** names an enabled plugin registers (per `plugin_skill_canonicals`) are excluded from `dangling_names()`; a test covers a retired loose copy whose plugin canonical still exists. Directory names that merely contain an item name (`docs/backlog-hygiene/`, `~/Projects/mini/`) are a separate, smaller false-positive class worth a note in the same fix.
- **Size:** S
- **Added:** 2026-09-22
...
- **Size:** M
- **Added:** 2026-08-12
- **Ready to start:** paste the handoff from [`docs/ideas/session-auto-close.md` § Handoff prompt](docs/ideas/session-auto-close.md#handoff-session-prompt-ready-to-paste). Run on Fable 5 at `xhigh`, **on the Mac** — the feasibility question is a hands-on Warp experiment, and the design questions need deciding before any code.
...
- **Size:** L
- **Added:** 2026-06-18
- **Ready to start:** a paste-able session prompt is written and waiting at [`docs/ideas/fleet-manifest-reconcile.md` § Handoff prompt](docs/ideas/fleet-manifest-reconcile.md#handoff-session-prompt-ready-to-paste). It carries the landmines from the 2026-07-28 purge that a cold session would otherwise re-hit. Run it on Fable 5 at `xhigh` — the five open design questions below need deciding before any code, and the doc's sizing assumption (7 repos) is already wrong (18).
...
- **Size:** L
- **Added:** 2026-08-20
- **Built 2026-08-20 (PR #102):** `skills/gauntlet/` + `agents/specifier.md`, `agents/gauntlet-coder.md`, `agents/mutation-hardener.md`. Four adversarial-review rounds, 19 findings, all fixed.
- **Pilot run 2026-08-20 — acceptance bar NOT met. Full report: [`docs/reports/2026-08-20-gauntlet-pilot.md`](docs/reports/2026-08-20-gauntlet-pilot.md).** On party-line's "quiet on purpose vs broken" story: stages 0–2 ran with real exit-code gates, **stage 3 never dispatched**, and against the control arm the gauntlet took **6.7× the wall-clock** (1 h 35 m vs 14 m 13 s), tied on every deterministic axis, and **lost** the unlabeled judge's comparison. Bob's own kill criterion — slower with no quality delta — is met.
...
- **Why:** Bob no longer imposes line-by-line TDD on agents (they revert to function-then-test anyway) — enforce the *values* via outcome gates instead. **Contradicts `/tdd-loop` and `superpowers:test-driven-development`**; the capture's job is to force that call. Full write-up in [`docs/ideas/values-not-disciplines-for-agents.md`](docs/ideas/values-not-disciplines-for-agents.md).
...
### [Improvement] /trim-context harness lane: audit built-in tool/feature token bloat
- **Why:** `/trim-context` audits only repo-side bloat; the harness side — feature blocks, tool definitions, bundled skills, connectors, paid on every request — is invisible to it, and Matt Pocock's 25K→8K numbers say it's the bigger prize. Full write-up in [`docs/ideas/trim-context-harness-lane.md`](docs/ideas/trim-context-harness-lane.md).
- **Acceptance:** `/trim-context` output gains a read-only harness report — the five documented disable settings with current values, bare-name-deniable tools, each annotated with observed usage — proposing but never applying disables.
- **Size:** M
- **Added:** 2026-08-22
