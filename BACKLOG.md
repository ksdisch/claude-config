...
- **Defects fixed 2026-08-21 (PR #108):** all four pipeline defects patched in `skills/gauntlet/SKILL.md` — G3 gates on changed lines rather than touched files, Stage 0's probe trial-mutates and passes only on `Killed`, Stage 0 requires a fresh session with all three agent types resolving, and STAGE-COMMITS waits for the agent's report. The fifth (contaminated arms) is fixed in the plan's new §4.1 three-arm protocol.
- **Next, if picked up:** re-run on a second story **in a fresh session**, three arms per [`docs/plans/agent-gauntlet-pipeline.md`](docs/plans/agent-gauntlet-pipeline.md) §4.1 — control, coder+gates with no specifier, full gauntlet — to measure the specifier's contribution directly rather than by inference. Story criteria: small, JS-only, unit-testable, a real backlog item, no attended leg. If the hardener finds nothing across two more stories, drop it from the relay and keep it as the standalone `AUDIT` auditor its own vision doc wanted.
