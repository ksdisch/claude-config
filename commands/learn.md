...
4. For each new skill, in the same commit (house rule): add a row to `docs/command-skill-reference.md` **and a card to `docs/usage-playbook.md`** (Run config · Reach for it when · Pairs well with, following that doc's own conventions), with the row's `config →` link pointing at the card's anchor. `scripts/check-doc-sync.py` blocks the push if they don't match.
