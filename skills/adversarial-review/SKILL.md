...
3. **Gate proposal (propose-first, updated 2026-08-22 — replaces the old auto-run + escape hatch).** This step applies when the skill is reached as the pre-merge gate (an autonomous merge is in play). When Kyle explicitly invoked the skill, the review itself is already his call — propose only the scope (SINGLE ROUND vs FULL LOOP, never SKIP) and continue.
   ...
   **Preflight — run the wired gates first, then cite them.** Before composing any recommendation, read the target repo's `CLAUDE.md` for a **"Wired gates (Preflight)"** note and run every command it lists, reading the output against this branch's diff. Cite the results in the recommendation as evidence: a clean Preflight is what lets you honestly argue the scope *down* (FULL LOOP → SINGLE ROUND → SKIP); a finding argues it up. Each note states what its gates' exit codes mean — non-zero is how some gates report a violation, and others always exit 0 because a ranking has no pass/fail — so never read a non-zero exit as broken tooling, and never read a gate you couldn't run as a pass: a gate that fails to run is cited as **Preflight incomplete**, which argues scope up, not down. Preflight never blocks by itself and never substitutes for the review (governance: claude-config's `docs/adr/0001-gates-earn-the-veto.md`; vocabulary: its `CONTEXT.md`). **No note, or a note listing no gates → one honest line saying the repo has nothing wired** — nothing else in this step changes.
