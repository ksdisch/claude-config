...
1. **Collapse the decisions into one spec.** Walk Decisions-so-far and zoom each ticket for its detail, then collapse them per the vendored `to-spec` skill — it is typed-only, so **read its process and template** ([../to-spec/SKILL.md](../to-spec/SKILL.md)) rather than invoking it, the same way this skill reads tracker.md — into the single document that states what is to be built and why: the decisions, not their transcripts. It lands where the repo already keeps specs; absent a convention, `docs/specs/<effort-slug>.md`. Link it from the map.
2. **Close the map** with a comment pointing at the spec.
3. **Route the next move.** Give Kyle 2–3 ranked options for what happens now — typically `to-tickets` to break the spec into tracer-bullet tickets with blocking edges, `backlog-hygiene` to sequence it against the rest of the backlog, `/autonomous-milestone` to build a well-specified chunk of it, or a straight builder session — each with a one-clause recommendation and an honest call on whether ultracode buys anything.
