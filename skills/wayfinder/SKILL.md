...
6. **Fire the research subagents.** For each **unblocked** `research` ticket just created — one whose blockers, if any, are already closed — claim it (invariant `claim-first`), then dispatch a subagent that calls the Skill tool with `research` to resolve it in parallel. **Blocked research tickets wait their turn**: step 5 wired the blocking graph precisely so it gates work, and a research answer produced on an unsettled premise still lands on the map's Decisions-so-far as though it were a decision on the route, where nothing reopens it when the blocker resolves differently.
   ...
   **Invariant `only-the-parent-appends`:** nor do those subagents append to the **map**. Every other resolution write touches a resource that is the ticket's alone, but Decisions-so-far is shared, and N agents appending to it at once is the one genuinely concurrent write in the whole skill. Each returns its gist instead, and this session appends it — which is also what makes `map-append-is-last-write`'s commit-each-edit-alone obligation reachable at all, since the agents that cannot commit are no longer the ones editing the map.

   **Append each gist the moment its subagent returns, not after all N are back.** Moving the map line into this process already stretches `resolve-order` across two processes; batching would stretch it across the whole fan-out, and this session — holding N research reports at once — is exactly where context runs out. A ticket closed with its answer but no map line is invisible: it passes the map-clear test, and the spec is written by walking Decisions-so-far, so the decision drops silently out of the deliverable. Appending on arrival keeps the serialization and shrinks the exposed window to one ticket.

   Per arrival, then: **re-read the map body, append that one gist, and commit that edit by itself** before collecting or dispatching anything else. That is `map-append-is-last-write`'s two obligations discharged one ticket at a time. Its *name* describes a session that appends once and charting is its named exception — do not read the name as licence to batch.
7. **Reconcile what didn't come back, then stop.** For every research ticket whose subagent didn't return — errored, timed out, returned nothing — **read the ticket on the tracker before deciding what to do**. A silent subagent is not the same thing as an unresolved ticket: the resolve procedure closes the ticket *before* returning, so a subagent that died in that window left the decision recorded and only the return missing. Classify on the ticket's state, never on the absence of a return.

   - **Still open** → release the claim. This is the case `claims-are-released` is about; without it the ticket sits claimed forever, off the frontier, and no later session surfaces it.
   - **Already closed or resolved** → **do not release** — releasing a closed ticket recovers nothing on either backend, since the frontier is built from *open* tickets. Instead read its resolution comment (Backend A) or its `## Answer` (Backend B) and **append that gist to Decisions-so-far yourself**. Otherwise the decision exists only in the ticket, the map-clear test passes without it, and the spec — written by walking Decisions-so-far — silently omits it.

   Charting hand-resolves nothing else. Report the map by name with its link, name the frontier tickets Kyle could take next, and give the two cases **separate sentences**: which tickets were unresolved and released, and which were *resolved by a subagent that didn't report back* and had their gist recovered onto the map. Collapsing the second into the first tells Kyle the opposite of what happened, and he is the last person who could have caught a missing decision.
