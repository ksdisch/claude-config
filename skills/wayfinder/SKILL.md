...
1. **Collapse the decisions into one spec.** Walk Decisions-so-far and zoom each ticket for its detail, then call the Skill tool with `to-spec` to synthesize the single document that states what is to be built and why — the decisions, not their transcripts. It lands where the repo already keeps specs; absent a convention, `docs/specs/<effort-slug>.md`. Link it from the map.
