...
6. **Fire the research subagents, and resolve their tickets yourself.** For each **unblocked** `research` ticket just created — one whose blockers, if any, are already closed — claim it (invariant `claim-first`), then dispatch a subagent that calls the Skill tool with `research` to investigate it in parallel. **Blocked research tickets wait their turn**: step 5 wired the blocking graph precisely so it gates work, and a research answer produced on an unsettled premise still lands on the map's Decisions-so-far as though it were a decision on the route, where nothing reopens it when the blocker resolves differently.
   ...
   **You perform the whole of `resolve-order` for each ticket, as its subagent returns.** One ticket at a time, in this session: post the answer as the resolution comment, close the ticket, re-read the map body, append that one gist, and commit that map edit by itself — then move to the next return. That keeps `resolve-order` atomic in one process (which is what it was always for), keeps the appends serialized, and discharges `map-append-is-last-write`'s two obligations once per ticket. Do **not** batch the resolutions to the end: this session is holding N research reports and is exactly where context runs out, and a ticket resolved but unrecorded is invisible — it passes the map-clear test, and the spec is written by walking Decisions-so-far.
