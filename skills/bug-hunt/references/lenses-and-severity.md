...
- **Parsing / external-call robustness** — malformed input (encoding, BOM, truncation), unhandled exceptions from external calls, missing timeouts / retries, path handling / traversal. (Error *swallowing* belongs to the silent-failure lens below — keep the slices apart.)
- **Silent failure / error handling** — swallowed errors (`catch {}`, `except: pass`), dangerous fallbacks that mask a real failure (`.catch(() => [])`), broken propagation (lost stack traces, generic rethrows that erase the error type, unawaited promises), log-and-continue handling where the caller proceeds as if nothing happened, and boundary calls with no timeout or rollback. **This lens has a dedicated finder:** give its dimension `agentType: 'silent-failure-hunter'` (see `agents/silent-failure-hunter.md`) rather than a generic finder prompt — it grades on the same rubric below, so its findings enter the verify stage unchanged.
