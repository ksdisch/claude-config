...
3. **How to get the most out of each.** Per item: what a good invocation brings, what wastes it, model and effort from the Planner/Builder ladder in `CLAUDE.md` (judgment-first → Fable, `xhigh`; mechanical → Sonnet), and traps. Traps always cover routing (`disable-model-invocation`, roster twins, prose that pulls a different skill with a similar name), artifact-or-none (does the session leave anything behind), and human-in-the-loop-only (does it block on answers). Typed usage counts and dates go here, per item.
