...
**Propose first (per the global review-gate convention, updated 2026-08-22).** Before running either branch below, give Kyle your honest take on which scale this diff needs — skip / inline adversarial read / full release-gate Workflow — and why (risk surface, behavioral vs. mechanical content, blast radius, test coverage). Interactive: pause for his call; he can veto in either direction and his call binds this merge. Unattended: decide yourself and record the decision + reasoning in the merge brief — with one floor: behavioral/agent-instruction diffs (`CLAUDE.md`, `skills/**`, `agents/**`, `commands/**`, hooks, `settings*.json`) get at least one independent review pass (a single-lens Workflow agent or one `adversarial-reviewer` round), never just the inline read. Then scale the review to the agreed call:
- **Substantial change** (new subsystem, many files, anything touching data integrity / privacy / money / auth, behavioral/agent-instruction files above all, or Kyle has opted into ultracode): the default recommendation is a **multi-agent release-gate Workflow** over the diff vs the default branch. Use independent adversarial lenses appropriate to the project (correctness/ship-blockers, security/privacy, plus any project-specific invariants — e.g. an "iron rule" or portability contract stated in `CLAUDE.md`), **verify each blocking/high finding independently**, then synthesize one **GO / NO-GO** with `decision`, `blockers[]`, `followUps[]`. This skill's instructions authorize the Workflow call.
...
- **Small/trivial change**: a thorough inline adversarial read of the diff is enough — but still actively try to *refute* the change, don't just skim it. Kyle can choose this branch for any diff interactively; unattended, behavioral/agent-instruction diffs never land here (the floor above — at least one independent review pass).
...
Start with the practical advice for getting the best result: when to run it (be present / reflective vs. fire-and-forget), prerequisites to pre-load, whether **voice mode** (`/converse`) suits it (great for conversational / extract-from-your-head work), rough time budget, whether to keep it collaborative vs. let it orchestrate, and what stays private/uncommitted. Then name the **natural session after that one**, and offer to generate its starter prompt when he's ready.

**End on the Run-config note** — required by `CLAUDE.md`'s Planner/Builder Protocol, which makes it the last thing addressed to Kyle. In this skill that puts it last outright, since §3.1's fenced block comes first; `/handoff-session` instead lets its audio note and then the paste-able block follow it, which the protocol allows — what the rule forbids is burying it mid-document or inside the paste. Either way it is prose *outside* the fenced starter prompt, never inside it (the fresh session can't set its own model/effort). It carries the recommended model + effort (per the protocol's ladder), a one-clause why, and the literal launch command — e.g. `claude --model claude-opus-5 --effort high`; quote bracketed 1M IDs (`'claude-opus-5[1m]'`); ultracode sessions launch with `--effort ultracode`. If you recommend ultracode, say in the note that Kyle must confirm dynamic workflows are enabled in `/config` — with the setting off, the flag parses clean, warns about nothing, and silently runs a plain xhigh single-agent session instead of the fan-out being promised. When the recommended model is Opus 5, apply the builder notes (`~/.claude/opus5-builder-notes.md` if present, else the vendored `.claude/opus5-builder-notes.md`) to §3.1's fenced starter prompt — the run-config note itself is unaffected; if neither copy exists, write the prompt normally.
