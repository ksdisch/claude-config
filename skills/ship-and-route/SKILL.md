...
**End on the Run-config note** — required by `CLAUDE.md`'s Planner/Builder Protocol, which makes it the last thing addressed to Kyle. In this skill that puts it last outright, since §3.1's fenced block comes first; `/handoff` orders the block after it instead, which the protocol allows. Either way it is prose *outside* the fenced starter prompt, never inside it (the fresh session can't set its own model/effort). It carries the recommended model + effort (per the protocol's ladder), a one-clause why, and the literal launch command — e.g. `claude --model claude-opus-5 --effort high`; quote bracketed 1M IDs (`'claude-opus-5[1m]'`); ultracode sessions launch with `--effort ultracode`. If you recommend ultracode, say in the note that Kyle must confirm dynamic workflows are enabled in `/config` — with the setting off, the flag parses clean, warns about nothing, and silently runs a plain xhigh single-agent session instead of the fan-out being promised.
