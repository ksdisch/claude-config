...
- **Do not append to the map.** Resolve the ticket itself — that's yours — but the map's Decisions-so-far is shared with every other agent running beside you, and N concurrent appends to it lose lines. Return the gist; the dispatching session appends all of them serially. This is invariant `only-the-parent-appends`, and it is the one step of `resolve-order` a research subagent does not perform.
