...
An artifact that never reaches `completed` can't be renamed, and the gate doesn't deadlock on it: an audio that ends `status:"failed"` (more likely in big batches) is logged in the sidecar and set aside — that closes the wave, because there is nothing there to rename. Offer to clear it (`studio_delete(artifact_id=…, confirm=True)`) and **ask before deleting** — deletion is irreversible, so `confirm=True` needs the user's word, not yours. Regenerating is optional and costs audio quota; when you do, the replacement is a **new artifact that runs the full fire → poll → rename sequence in its own wave** — folded into the next batch, or as a wave of its own if the failure landed in the last one. Never let a regenerate ride out past the wave gate — it lands auto-titled minutes later, which is the exact leftover this step exists to prevent. Episodes blocked by quota were never created at all, so they're deferrals (see below), not unrenamed artifacts.
...
- **Rename only after completion** — auto-titles overwrite early renames. Rename on `status:"completed"` only — `unknown`+`audio_url` is the lag window, not the green light. And renaming is **mandatory and terminal per wave**: a wave isn't done until every artifact in it that reached `completed` carries its `Ep N —` title, so never fire the next batch (or call the step complete) with auto-titled artifacts *from that wave* left behind. A `failed` artifact is logged rather than renamed, so it doesn't block the wave (deleting it is offered, never automatic — ask first); a regenerate is a new artifact that runs the full sequence in its own wave.
