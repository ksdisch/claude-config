...
Surface the URL (and the `?variant=` keys), say what each variant trades away, name the one you'd pick and why — then **stop**. Kyle flips through whenever he gets to it. The interesting feedback is usually **"I want the header from B with the sidebar from C"**, which is the actual design he wants, and it's feedback you can only get by waiting for it.

Do not proceed to step 6 on your own reading of your own variants. See [The verdict is Kyle's, never yours](SKILL.md#the-verdict-is-kyles-never-yours).

### 6. Capture the answer and clean up

Once **Kyle** has picked a variant, capture the answer (which variant and why), then capture the prototype the way the [SKILL](SKILL.md) describes. Fold the winner into the real code and move the rest onto the throwaway branch, not into main:
