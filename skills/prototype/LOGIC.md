...
Then **stop**. Driving your own walkthroughs and concluding the model looks fine is not the demo answering its question — it's the demo answering nobody. See [The verdict is Kyle's, never yours](SKILL.md#the-verdict-is-kyles-never-yours).

### 5. Capture the answer and the prototype

Once a human has driven it and said what they found, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML shell rides along to the throwaway branch that keeps the prototype as a primary source, and being one self-contained file, it stays trivially re-runnable there.
