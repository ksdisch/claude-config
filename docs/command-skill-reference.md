...
| `reaper` | Proposes closing the predecessor session that launched this one, once its handoff note has been consumed (R2) — one SIGTERM after a live identity re-verification and one yes/no; never touches a transcript or a note. · [config →](usage-playbook.md#reaper) |
