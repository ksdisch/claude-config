...
| [`mock-panel`](../skills/mock-panel/SKILL.md) | Live mock interview for a real upcoming job interview — Claude plays the panel in character, grounded in the vault's prep dossier/application/JD, one question at a time with realistic follow-ups, then a scorecard debrief; session logs make later mocks re-test the weak answers. · [config →](usage-playbook.md#mock-panel) |
