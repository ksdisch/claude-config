...
| [`/handoff-session`](../commands/handoff-session.md) | Generates a self-contained handoff prompt you can paste into a fresh session to continue work without losing context. Also prints a plain-English "what's next & why" briefing. · [config →](usage-playbook.md#handoff-session) |
...
| `call-brief` | Full spoken briefing for one dial — who they are, where the thread stands, pitch type, the opener, full-sentence objection responses, the voicemail line. One contact per run; never logs or sends. · [config →](usage-playbook.md#call-brief) |
