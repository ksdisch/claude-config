...
| [`visual-summary`](../skills/visual-summary/SKILL.md) | Turns a conversation, a branch's diff, or an uncommitted worktree into one self-contained HTML page that's read at a glance — three to five charts and diagrams, one headline, no second block of prose. Saves to `~/Projects/_visual-summaries/` and publishes as an Artifact every run. · [config →](usage-playbook.md#visual-summary) |
