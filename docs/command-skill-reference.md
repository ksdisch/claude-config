...
| [`to-spec`](../skills/to-spec/SKILL.md) | Turns the current conversation into a spec (`/to-spec`) — no interview, pure synthesis into a problem/user-stories/decisions template; lands where the repo keeps specs (`docs/specs/` absent a convention), with an optional guarded publish to the tracker. · [config →](usage-playbook.md#to-spec) |
| [`to-tickets`](../skills/to-tickets/SKILL.md) | Breaks a plan, spec, or conversation into tracer-bullet vertical-slice tickets with explicit blocking edges (`/to-tickets`) — one file per ticket under `docs/tickets/`, or GitHub Issues with native dependencies; expand–contract sequencing for wide refactors. · [config →](usage-playbook.md#to-tickets) |
| [`implement`](../skills/implement/SKILL.md) | Builds a piece of work from a spec or ticket set (`/implement`) — TDD at pre-agreed seams, typechecks along the way, full suite at the end, feature branch + PR with the adversarial-review gate. Third link of to-spec → to-tickets → implement. · [config →](usage-playbook.md#implement) |
...
| [`triage`](../skills/triage/SKILL.md) | Moves tracker issues (and, when opted in, external PRs) through a triage state machine (`/triage`) — categorise, verify claims, grill when fuzzy, write agent-ready briefs, and keep a `docs/out-of-scope/` knowledge base of rejected requests. · [config →](usage-playbook.md#triage) |
