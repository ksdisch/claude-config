...
python3 ledger.py \
  ...
  -o /tmp/ledger.json
python3 -c "import json;d=json.load(open('/tmp/ledger.json'));print(len(d['figures']),'slots');print('nums ok:',[f['num'] for f in d['figures']]==list(range(1,95)));print('with url:',sum(1 for f in d['figures'] if f['existing_url']))"
```
Expected:
```
94 figure slots
...
This is the real acceptance check for this task — 94 slots, numbered 1..94 with no gaps, exactly one already carrying a URL (Figure 4).
...
## Remaining stages (to be expanded before execution)

- **Stage 6 — `inject_html.py`:** `.figure-placeholder` div → `<figure class="paper-figure">` + base64 `<img>`; `.paper-figure` CSS via existing theme variables; singleton lightbox coordinating with the gloss popover and glossary panel.
- **Stage 7 — `contactsheet.py`:** 320px thumbnails, numbered, captions, Tier-1 failures marked.
- **Stage 8 — `SKILL.md` + `references/capture-recipes.md`:** orchestration prose; exact Playwright invocations; PDF sources report-and-stop.
- **Stage 9 — amendments:** `paper-gloss` self-containment rule + image handling, `paper-eli5` Phase 3 counting, `docs/command-skill-reference.md` row.
- **Stage 10 — Slice A:** the 10 static figures end-to-end.
- **Stage 11 — Slice B:** 10 interactive figures via Playwright.
- **Stage 12 — Slice C:** full 94, contact-sheet gate, delivery.
