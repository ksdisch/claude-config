...
- **Fog**: each Not-yet-specified bullet is a dashed node in one final column right of the edge box. It gets a dashed edge from a ticket when a `waits on` clause in the bullet (up to the end of its sentence) names ticket numbers or contains a ticket's full title (case-insensitive); a title cited elsewhere in the bullet is context and draws nothing (review F1, 2026-09-22). No match → drawn unattached. Multiple matches → multiple edges.
